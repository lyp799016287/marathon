    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        _createHeaderCell: function (column)
        {
			...
			if (column.issortshow)
			{
				jcell.html("<div class='l-grid-hd-cell-inner'><span class='l-grid-hd-cell-text'></span><span class='l-grid-sortable'>&nbsp;&nbsp;</span></div>");
			}
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
				...
				if(g.columns.length > 0){
					var column = g.columns[0], content = this._getCellContent(item, column);
					content = content.replace(/\s/g, "");
					var dateMatch = null;
					if(column.name.indexOf('date')!=-1 && (dateMatch = content.match(/(\d{4})(\/|-)(\d{1,2})(\/|-)(\d{1,2})/))){
						var date = new Date(dateMatch[1], dateMatch[3] - 1, dateMatch[5]), day = date.getDay();
						if(day == 0 || day == 6)
							style.push("background:#fff8d2;font-weight:bold");
					}
				}
                $(g.columns).each(function (columnindex, column)
                {
					...
					if(style.length > 0) gridhtmlarr.push(style.join(";") + ";");
        ...
        _onClick: function (e)
        {
            ...
            {
                $("#" + g.id).find(".l-grid-hd-cell-sort").each(function(){
                    if($(this).parents("td").attr("columnname") != src.column.name){
                        $(this).attr("class","l-grid-sortable");
                    }
                });
                ...
                {
                    hcell.removeClass("l-grid-hd-cell-desc").addClass("l-grid-hd-cell-asc");
					hcell.find('.l-grid-sortable').remove();
