...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
			...
			if(p.initValue){
				var temp_data = p.initValue.split("-");
				var nowDate = new Date(temp_data[0], temp_data[1] - 1, temp_data[2]);
			}else if(p.Value){
				var temp_data = p.Value.split("-");
				var nowDate = new Date(temp_data[0], temp_data[1] - 1, temp_data[2]);
			}else{
				var nowDate = new Date();
			}		
            ...
            *************/
            ...
            g.buttons.btnToday.click(function ()
            {
                ...
                g.currentDate = {
                    year: g.now_today.year,
                    month: g.now_today.month,
                    day: g.now_today.day,
                    date: g.now_today.date
                };
                g.selectedDate = {
                    year: g.now_today.year,
                    month: g.now_today.month,
